    ...
    if(isset($_POST['submit'])){
      ...
      $name = addslashes($_POST['username']);
  			$user_result = sort_username($name);
  				if($user_result == 3){
  					$mes_user = " Novo nome de usuário deve ser alfanumérico!";
  				} else {
            $user_result = true;
            $name_new = $name;
          }
      ...
      $email = addslashes($_POST['email']);
  			...
  			if($email_result == 3){
  				$mes_email = "Por favor digite um email válido!";
  			} else {
          $email_result = true;
          $email_new = $email;
        }
      $password = addslashes($_POST['pwd']);
        $pwd_result = sort_password($password);
            if($pwd_result == 2){
              $mes_pwd = "A nova senha deve ter mais de 6 caracteres!";
            } else {
              $pwd_result = true;
            ...
            $user_email = $user_data[0][2];
              $user_pwd = $user_data[0][3];
                $user_img = $user_data[0][4];
                  $btn = 'success';
                    $mes_head = 'As alterações foram atualizadas na sua conta. <a href="account.php">View Account</a>';
                      $mes_user = '';
                        $mes_email = '';
                          $mes_pwd = '';
  }
